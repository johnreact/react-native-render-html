...
import { TText, TBlock, TNode, TPhrasing } from '@native-html/transient-render-engine';
...
const TDefaultBlockRenderer: TDefaultRenderer<TBlock> = renderBlockContent.bind(null);
TDefaultBlockRenderer.displayName = 'TDefaultBlockRenderer';

const TDefaultPhrasingRenderer: TDefaultRenderer<TPhrasing> = renderTextualContent.bind(null);
TDefaultPhrasingRenderer.displayName = 'TDefaultPhrasingRenderer';

const TDefaultTextRenderer: TDefaultRenderer<TText> = renderTextualContent.bind(null);
...
const TNodeRenderer = memo(function MemoizedTNodeRenderer(
  ...
  const {
    tnode,
    propsFromParent = { collapsedMarginTop: null } // Set default value here
  } = props;
  ...
  const { assembledProps, Renderer } = useAssembledCommonProps(tnodeProps, renderer as any);
  switch (tnode.type) {
    case 'empty':
    ...
    case 'phrasing':
      ...
      if (
        ...
        return React.createElement(TNodeChildrenRenderer, { tnode: props.tnode });
      }
